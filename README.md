...
2. Choose one of the two downloads:
   - **`Sakoon.zip`** — portable. Unzip and run `Sakoon.exe`.
     Nothing is installed, and nothing is added to Windows startup.
   - **`Sakoon-Setup.zip`** — installer. Unzip and run `Sakoon-Setup.exe`.
     It installs per-user (no admin), adds Start Menu and uninstall entries,
     and — ticked by default — creates a `shell:startup` shortcut so Sakoon
     starts with Windows. Untick that during setup if you don't want it.
3. Compare the download's SHA-256 hash with `SHA256SUMS.txt` from the same
   release before extracting.
4. Run Sakoon. Administrator access is not required. The native window opens
   and a tray icon appears. The app itself never adds anything to startup —
   that is the installer's job, or yours via Windows Settings ▸ Apps ▸
   Startup.
